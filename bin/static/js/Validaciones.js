...
$(document).ready(
    function()
    {
        ...
        $(".RFCMoral").inputmask(
            'Regex',
            {
                regex: "^[a-zA-Z]{3}[0-9]{6}[0-9a-z�A-Z�]{3}$",
...
function ValidarCampos(Accion) {
    ...
    if(valido=="true")
    {
        ...
        bootbox.alert({
            message: '<p style="overflow: hidden; float: left;" class="">' + '<img style="margin: -220px 0px -240px 0px;" src="./img/messages-g.png" /></p>'
            + '<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&#161;Atenci&oacute;n&#33; </p>' + '<center><label>Llenar los campos requeridos' + '</label></center>'
        });
}
...
function CargaComboRestringido(Combo, nomCatalago,Valor){
    try {
        var active = dataBase.result;
        var data = active.transaction([nomCatalago], "readonly");
        var object = data.objectStore(nomCatalago);
